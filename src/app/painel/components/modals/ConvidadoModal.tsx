...
export function ConvidadoModal({ row, hasVisibilidade, onSubmit, onCancel }: { row?: Row; hasVisibilidade?: boolean; onSubmit: any; onCancel: any }) {
  ...
  return <form method="POST" className="form-modal" onSubmit={onSubmit}>
    ...
    <div className="secao-form">
      ...
      <div className="linha-form"><div className="grupo-input"><label>E-mail</label><input type="email" name="email" placeholder="email@exemplo.com" defaultValue={row?.email || ""} /><small className="form-hint">Deixe em branco para remover o e-mail do convidado.</small></div></div>
    </div>
      ...
      <div className="grupo-input spaced">
        ...
        <div id="container-nomes-lista">
          {nomes.map((item, idx) => <div className="linha-nome-dinamico" key={idx}>
            ...
            <select className="status-dinamico" value={item.status} onChange={(e) => update(idx, { status: e.target.value })}><option value="pendente">Pendente</option><option value="confirmado">Confirmado</option><option value="recusado">Ausente</option></select>
          </div>)}
        </div>
